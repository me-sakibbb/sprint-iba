...
serve(async (req) => {
    ...
    try {
        ...
        const totalRemaining = subjectStats.reduce((sum: any, s: any) => sum + s.totalRemaining, 0);
        const totalAnswered = subjectStats.reduce((sum: any, s: any) => sum + s.totalAnswered, 0);
        const overallProgress = Math.round((totalAnswered / (totalAnswered + totalRemaining)) * 100);

        // Build topic summary
        const subjectSummary = subjectStats.map((s: any) => {
        ...
        const response = await fetch(`${GEMINI_API_URL}?key=${apiKey}`, {
            ...
            body: JSON.stringify({
                ...
                generationConfig: {
                    temperature: 0.7,
                    topP: 0.95,
                    maxOutputTokens: 32000  // Increased for full 30 days
                }
            })
        });
